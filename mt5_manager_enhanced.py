...
class MT5ManagerApp:
    ...
    def create_environment_tab(self):
        ...
        self.notebook.add(env_frame, text="🔧 环境配置")

        # Python 环境信息
        python_frame = ttk.LabelFrame(env_frame, text="Python 环境", padding=10)
        python_frame.pack(fill=tk.X, padx=10, pady=5)

        # 显示当前 Python 信息
        if self.python_exe:
            python_info_label = ttk.Label(
                python_frame,
                text=f"当前 Python: {self.python_exe}",
                font=("Microsoft YaHei", 12)
            )
        else:
            python_info_label = ttk.Label(
        ...
        python_info_label.pack(pady=5)

        ttk.Button(
            python_frame,
            text="安装/更新 Python 运行时",
            command=self.show_install_python_dialog,
            width=25
        ).pack(pady=5)
        ...
        # 路径配置
        paths_frame = ttk.LabelFrame(env_frame, text="路径配置", padding=10)
        paths_frame.pack(fill=tk.X, padx=10, pady=5)

        row = 0
        ttk.Label(paths_frame, text="程序目录:").grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(paths_frame, text=str(self.base_dir)).grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        ttk.Label(paths_frame, text="配置目录:").grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(paths_frame, text=str(self.config_dir)).grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        ttk.Label(paths_frame, text="日志目录:").grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(paths_frame, text=str(self.logs_dir)).grid(row=row, column=1, sticky=tk.W, pady=2)
    ...
    def create_status_bar(self):
        ...
        python_version = sys.version.split()[0] if self.python_exe else "未安装"
        ttk.Label(right_status, text=f"Python: {python_version}").pack(side=tk.LEFT, padx=5)
    ...
    def start_master(self):
        ...
        if not self.python_exe:
            messagebox.showerror("错误", "Python 未安装，请先安装 Python 运行时")
            return
        ...
        try:
            ...
            self.master_process = subprocess.Popen(
                [self.python_exe, str(script_path), "--config", str(config_path)],
...
安装完成后，您将能够使用所有功能。
        ...
        ttk.Button(
            button_frame,
            text="下载并安装 Python",
            command=lambda: self.install_python_runtime(dialog),
            width=20
        ).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(
            button_frame,
            text="手动指定 Python 路径",
            command=lambda: self.select_python_path(dialog),
            width=20
        ).pack(side=tk.LEFT, padx=5)
    ...
    def install_python_runtime(self, dialog):
        """安装 Python 运行时"""
        if sys.platform != 'win32':
            messagebox.showinfo("提示", "此功能仅在 Windows 上可用")
            return

        # 启动安装进程
        install_script = self.base_dir / "install_python.bat"
        if install_script.exists():
            ...
            dialog.destroy()
            messagebox.showinfo("提示", "Python 安装程序已在新窗口中启动")
        else:
            messagebox.showerror("错误", "找不到安装脚本")

    def select_python_path(self, dialog):
        """手动选择 Python 路径"""
        python_path = filedialog.askopenfilename(
            title="选择 Python 可执行文件",
            filetypes=[("Python 可执行文件", "python.exe pythonw.exe"), ("所有文件", "*.*")]
        )
        if python_path:
            self.python_exe = python_path
            dialog.destroy()
            messagebox.showinfo("成功", f"已设置 Python 路径: {python_path}")

    def install_dependencies(self):
        """安装依赖包"""
        if not self.python_exe:
            messagebox.showerror("错误", "请先安装 Python")
            return

        try:
            # 创建日志文件
            install_log = self.logs_dir / "install_deps.log"
            ...
            self.update_status("依赖包安装完成")

        except subprocess.CalledProcessError as e:
            messagebox.showerror("错误", f"依赖包安装失败: {e}")
            self.update_status(f"依赖包安装失败: {e}")
        except Exception as e:
